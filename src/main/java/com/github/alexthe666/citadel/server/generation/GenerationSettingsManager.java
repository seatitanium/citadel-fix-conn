...
public class GenerationSettingsManager {
   ...
   public static void onChunkPopulate(
      ChunkGenerator chunkGenerator, WorldGenLevel level, ChunkAccess access, StructureFeatureManager featureManager, BiomeSource biomeSource
   ) {
      ...
      ChunkPos.rangeClosed(sectionpos.chunk(), 1).forEach(p_196730_ -> {
         ...
         for (LevelChunkSection levelchunksection : chunkaccess.getSections()) {
            levelchunksection.getBiomes().getAll(biomes -> set.add((Biome)biomes.value()));
         }
      });
      ...
      for (Biome biome : set) {
         for (Pair<String, PlacedFeature> pair : REGISTRY) {
            if (((String)pair.getA()).equals(biome.getRegistryName().toString())) {
               ...
               ((PlacedFeature)pair.getB()).placeWithBiomeCheck(level, chunkGenerator, random, blockpos);
            }
         }
      }
   }
