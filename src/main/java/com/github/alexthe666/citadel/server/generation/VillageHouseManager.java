   ...
   public static void addAllHouses(RegistryAccess registryAccess) {
      try {
         for (String villagePool : VILLAGE_REPLACEMENT_POOLS) {
            StructureTemplatePool pool = (StructureTemplatePool)registryAccess.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
               .getOptional(new ResourceLocation(villagePool))
               .orElse(null);
            if (pool != null) {
               String poolName = pool.getName().toString();

               for (Pair<String, Consumer<StructureTemplatePool>> pair : REGISTRY) {
                  if (((String)pair.getFirst()).equals(poolName)) {
