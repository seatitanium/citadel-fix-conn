   ...
   public void tick() {
      if (this.operation == Operation.STRAFE) {
         ...
         if ((!(this.mob instanceof ICustomCollisions) || !((ICustomCollisions)this.mob).canPassThrough(blockpos, blockstate, voxelshape))
            && (
               d2 > (double)this.mob.maxUpStep && d0 * d0 + d1 * d1 < (double)Math.max(1.0F, this.mob.getBbWidth())
                  || !voxelshape.isEmpty()
      ...
      if (pathnavigator == null) {
         ...
         return nodeprocessor == null
            || nodeprocessor.getBlockPathType(
                  this.mob.level,
                  Mth.floor(this.mob.getX() + (double)p_234024_1_),
                  Mth.floor(this.mob.getY()),
                  Mth.floor(this.mob.getZ() + (double)p_234024_2_)
               )
