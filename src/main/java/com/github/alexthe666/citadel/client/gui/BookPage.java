...
public class BookPage {
   ...
   public static BookPage deserialize(Reader readerIn) {
      return (BookPage)GsonHelper.fromJson(GSON, readerIn, BookPage.class);
   }
   ...
   public String generateTitle() {
      return this.translatableTitle != null ? I18n.get(this.translatableTitle, new Object[0]) : this.title;
   }
   ...
   public static class Deserializer implements JsonDeserializer<BookPage> {
      public BookPage deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
         ...
         LinkData[] linkedPageRead = (LinkData[])GsonHelper.getAsObject(jsonobject, "linked_page_buttons", new LinkData[0], p_deserialize_3_, LinkData[].class);
         EntityLinkData[] linkedEntitesRead = (EntityLinkData[])GsonHelper.getAsObject(
            jsonobject, "entity_buttons", new EntityLinkData[0], p_deserialize_3_, EntityLinkData[].class
         );
         ItemRenderData[] itemRendersRead = (ItemRenderData[])GsonHelper.getAsObject(
            jsonobject, "item_renders", new ItemRenderData[0], p_deserialize_3_, ItemRenderData[].class
         );
         RecipeData[] recipesRead = (RecipeData[])GsonHelper.getAsObject(jsonobject, "recipes", new RecipeData[0], p_deserialize_3_, RecipeData[].class);
         TabulaRenderData[] tabulaRendersRead = (TabulaRenderData[])GsonHelper.getAsObject(
            jsonobject, "tabula_renders", new TabulaRenderData[0], p_deserialize_3_, TabulaRenderData[].class
         );
         EntityRenderData[] entityRendersRead = (EntityRenderData[])GsonHelper.getAsObject(
            jsonobject, "entity_renders", new EntityRenderData[0], p_deserialize_3_, EntityRenderData[].class
         );
         ImageData[] imagesRead = (ImageData[])GsonHelper.getAsObject(jsonobject, "images", new ImageData[0], p_deserialize_3_, ImageData[].class);
         String readParent = "";
