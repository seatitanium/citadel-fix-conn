...
public class LightningBoltData {
   ...
   public List<LightningBoltData.BoltQuads> generate() {
      ...
      for (int i = 0; i < this.count; i++) {
         ...
         while (!drawQueue.isEmpty()) {
            LightningBoltData.BoltInstructions data = drawQueue.poll();
            ...
            if (!data.isBranch) {
               drawQueue.add(
                  new LightningBoltData.BoltInstructions(segmentEnd, progress, perpendicularDist, (LightningBoltData.QuadCache)quadData.getRight(), false)
               );
            } else if (this.random.nextFloat() < this.renderInfo.branchContinuationFactor) {
               drawQueue.add(
                  new LightningBoltData.BoltInstructions(segmentEnd, progress, perpendicularDist, (LightningBoltData.QuadCache)quadData.getRight(), true)
               );
            }
   ...
   public static class BoltRenderInfo {
      ...
      private LightningBoltData.RandomFunction randomFunction = LightningBoltData.RandomFunction.GAUSSIAN;
      private LightningBoltData.SpreadFunction spreadFunction = LightningBoltData.SpreadFunction.SINE;
   ...
   public interface SpreadFunction {
      ...
      LightningBoltData.SpreadFunction SINE = progress -> (float)Math.sin(Math.PI * (double)progress);
