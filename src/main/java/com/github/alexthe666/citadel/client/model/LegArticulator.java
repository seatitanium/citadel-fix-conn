...
public final class LegArticulator {
   public static void articulateQuadruped(
      LivingEntity entity,
      LegSolverQuadruped legs,
      AdvancedModelBox body,
      AdvancedModelBox lowerBody,
      AdvancedModelBox neck,
      AdvancedModelBox backLeftThigh,
      AdvancedModelBox backLeftCalf,
      AdvancedModelBox[] backLeftFoot,
      AdvancedModelBox backRightThigh,
      AdvancedModelBox backRightCalf,
      AdvancedModelBox[] backRightFoot,
      AdvancedModelBox frontLeftThigh,
      AdvancedModelBox frontLeftCalf,
      AdvancedModelBox[] frontLeftFoot,
      AdvancedModelBox frontRightThigh,
      AdvancedModelBox frontRightCalf,
      AdvancedModelBox[] frontRightFoot,
      float rotBackThigh,
      float rotBackCalf,
      float rotBackFoot,
      float rotFrontThigh,
      ...
      if (heightBackLeft > 0.0F || heightBackRight > 0.0F || heightFrontLeft > 0.0F || heightFrontRight > 0.0F) {
         ...
         float tilt = (float)(Mth.atan2((double)(bodyLength * sc), (double)(backAvg - frontAvg)) - (Math.PI / 2));
   ...
   private static void articulateLegPair(
      ...
      float avg,
      float offsetY,
      AdvancedModelBox leftThigh,
      AdvancedModelBox leftCalf,
      AdvancedModelBox[] leftFoot,
      AdvancedModelBox rightThigh,
      AdvancedModelBox rightCalf,
      AdvancedModelBox[] rightFoot,
      float rotThigh,
      float rotCalf,
      float rotFoot
   ) {
      ...
      leftThigh.rotationPointY = leftThigh.rotationPointY + 16.0F / sc * (Math.max(heightLeft, avg) + offsetY);
      rightThigh.rotationPointY = rightThigh.rotationPointY + 16.0F / sc * (Math.max(heightRight, avg) + offsetY);
      ...
      for (AdvancedModelBox part : rightFoot) {
         part.rotateAngleX = part.rotateAngleX - rotFoot * Math.min(0.0F, heightRight - heightLeft);
      }

      for (AdvancedModelBox part : leftFoot) {
         part.rotateAngleX = part.rotateAngleX - rotFoot * Math.min(0.0F, heightLeft - heightRight);
      }
   }
   ...
   private static float getScale(LivingEntity entity) {
      return entity instanceof IScaleable ? ((IScaleable)entity).getScaleForLegSolver() : entity.getBbWidth();
   }
}
